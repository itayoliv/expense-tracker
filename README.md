...
Update `expense_tracker.__version__` and `CHANGELOG.md` together for each release or build. Mark the commit with an annotated Git tag matching `__version__` (for example `v0.4.2` or `v0.4.2+2`); keep version out of everyday commit messages.
