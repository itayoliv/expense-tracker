...
def test_group_unsorted_names_collapses_same_description():
    from expense_tracker.integrations.gpt_sort import group_unsorted_names
...
def test_parse_gpt_json_matches_en_he_and_id():
    from expense_tracker.integrations.gpt_sort import parse_gpt_json
...
def test_ask_openai_http_falls_back_to_curl_when_avg_blocks(monkeypatch):
    ...
    import expense_tracker.integrations.gpt_sort as gpt_sort
...
def test_gpt_sort_groups_and_applies_only_unsorted(client, monkeypatch):
    ...
    monkeypatch.setattr("expense_tracker.integrations.gpt_sort.ask_openai", fake_ask)
    monkeypatch.setattr("expense_tracker.integrations.gpt_sort.get_api_key", lambda: "sk-test")
...
def test_gpt_sort_ignores_unknown_category_and_name(client, monkeypatch):
    _add_txn(client, "פז", 50, "debit", "2026-08-01")
    monkeypatch.setattr(
        "expense_tracker.integrations.gpt_sort.ask_openai",
...
def test_manual_patch_clears_gpt_tag(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "expense_tracker.integrations.gpt_sort.ask_openai", lambda *_a, **_k: {"Shopping": ["ZARA"]}
    )
    monkeypatch.setattr("expense_tracker.integrations.gpt_sort.get_api_key", lambda: "sk-test")
...
def test_dashboard_shows_gpt_badge_and_sort_button(client, monkeypatch):
    ...
    txn_js = client.get("/static/js/transactions-banner.js").get_data(as_text=True)
...
def test_save_and_clear_openai_key(client, tmp_path):
    from expense_tracker.integrations.gpt_sort import get_api_key, openai_key_path
